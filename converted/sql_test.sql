...
clv_base AS (
  SELECT 
    co.customer_id,
    co.total_orders,
    co.lifetime_revenue AS total_revenue,
    co.avg_order_value,
    cc.days_as_customer,
    CAST((co.last_order_date - co.first_order_date) AS INT) AS customer_lifespan_days
...
merged_data AS (
  SELECT 
    ...
    co.first_order_date,
    co.last_order_date,
    CAST((CURRENT_DATE - co.last_order_date) AS INT) AS days_since_last_order,
