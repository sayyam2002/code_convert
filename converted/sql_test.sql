...
filtered_customers AS (
    SELECT customer_id, email, signup_date, first_name, last_name, phone, address, city, state, country,
           birth_date, gender, income_bracket, customer_segment
...
aggregated_orders AS (
    SELECT co.customer_id,
           ...
           COALESCE(comp.total_revenue, 0) AS lifetime_revenue,
           ...
           COALESCE(o30.revenue_last_30_days, 0) AS revenue_last_30_days,
           COALESCE(o90.revenue_last_90_days, 0) AS revenue_last_90_days,
           COALESCE(o30.orders_last_30_days, 0) AS orders_last_30_days
    ...
    LEFT JOIN orders_last_30_days o30 ON co.customer_id = o30.customer_id
    LEFT JOIN orders_last_90_days o90 ON co.customer_id = o90.customer_id
),
...
rfm_base AS (
    SELECT customer_id,
           CAST((CURRENT_DATE - last_order_date) AS INT) AS recency_days,
...
rfm_scores AS (
    SELECT customer_id,
           ...
           frequency,
           monetary,
           CASE NTILE(5) OVER (ORDER BY recency_days DESC)
...
clv_base AS (
    SELECT ao.customer_id,
           ao.total_orders,
           ao.lifetime_revenue AS total_revenue,
           ao.avg_order_value,
           CAST((ao.last_order_date - ao.first_order_date) AS INT) AS customer_lifespan_days,
           cc.days_as_customer
...
merged_data AS (
    SELECT cc.customer_id,
           ...
           COALESCE(ao.completed_orders, 0) AS completed_orders,
           COALESCE(ao.cancelled_orders, 0) AS cancelled_orders,
           COALESCE(ao.returned_orders, 0) AS returned_orders,
           COALESCE(ao.lifetime_revenue, 0) AS lifetime_revenue,
           COALESCE(ao.avg_order_value, 0) AS avg_order_value,
           ao.first_order_date,
           ao.last_order_date,
           CASE WHEN ao.last_order_date IS NOT NULL THEN CAST((CURRENT_DATE - ao.last_order_date) AS INT) END AS days_since_last_order,
           COALESCE(ao.revenue_last_30_days, 0) AS revenue_last_30_days,
           COALESCE(ao.revenue_last_90_days, 0) AS revenue_last_90_days,
           COALESCE(ao.orders_last_30_days, 0) AS orders_last_30_days,
           tc.top_category_1,
...
SELECT *
FROM final_report
ORDER BY estimated_clv_3yr DESC NULLS LAST, lifetime_revenue DESC;
